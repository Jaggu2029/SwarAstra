...
const ProgressReport = () => {
  ...
  useEffect(() => {
    ...
    if (role === 'teacher' || role === 'parent') {
      fetchLinkedStudents().then(linked => {
        setStudents(linked || []);
        const id = linked && linked.length > 0 ? linked[0].id : session?.user?.id;
  ...
  if (!session) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
        ...
        <p className="text-gray-400">Please <Link to="/login" className="text-primary-sign underline">log in</Link> to view your progress.</p>
      </div>
    );
  }
  if (!profile) return <div className="text-center py-20 text-gray-400 animate-pulse">Loading profile...</div>;
  ...
  return (
    <div className="pb-16 animate-fade-in">
      <div className="flex items-center gap-4 mt-4 mb-8">
        ...
        <button onClick={() => loadData(selectedStudent)} className="p-2 glass-card hover:bg-white/10 rounded-full transition-colors" title="Refresh">
