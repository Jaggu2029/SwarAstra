...
const SciencePractice = () => {
  ...
  const [levels, setLevels] = useState(
    scienceLevels.map(l => ({
      level: l.levelNumber,
      title: locale === 'en' ? l.levelName_en : l.levelName_en, // Using English title per user request for headings
      unlocked: true,
      best_accuracy: 0
    }))
  );
  ...
  const loadProgress = async () => {
    ...
    setLevels(scienceLevels.map(l => {
      ...
      return {
        level: l.levelNumber,
        title: locale === 'en' ? l.levelName_en : l.levelName_en, // Force English per request
        unlocked: true,
  ...
  const handleNextClick = () => {
    // If they got it wrong, let them click Next manually instead of waiting
    if (currentIndex + 1 < QUESTIONS_PER_LEVEL) {
      setCurrentIndex(prev => prev + 1);
      setSelectedAnswer(null);
      setIsCorrect(null);
